# Araştırma Raporu: Emisyon Raporlama ve Karbon Kredisi Entegrasyonu
## Piri Reis Üniversitesi Blockchain Akademisi ve Web3 Hub
**Bölüm:** Gemi Makineleri İşletme Mühendisliği  
**Ders Kimliği:** PRU-ME-3  

---

## 1. Giriş ve Sektörel Kriz Analizi
Geleneksel endüstriyel altyapıda karşılaşılan en büyük dar boğazlardan biri, operasyonların merkezi veritabanlarına, fiziksel evrak dolaşımına ve çok sayıdaki aracı kuruma bağımlı olmasıdır. Gemi Makineleri İşletme Mühendisliği disiplini özelinde bu kriz, kendisini doğrudan şu sorunlarla hissettirmektedir: **Uluslararası denizcilikte (IMO/EU ETS) gemilerin emisyon raporlarında elle tahrifat yapılması ve yeşil aklama (greenwashing) yaygındır.**
...
Bu vizyon, Emisyon Raporlama ve Karbon Kredisi Entegrasyonu modelini sadece teknik bir güncelleme olmaktan çıkarıp, tüm sektörü dönüştüren stratejik bir Web3 hamlesine dönüştürür.
...
/**
 * @title Emisyon Raporlama ve Karbon Kredisi Entegrasyonu Mülkiyet ve Kayıt Akıllı Sözleşmesi
 * @notice Piri Reis Üniversitesi Blockchain Akademisi Eğitim Kaynağı.
 */
contract EmisyonRaporlamaveKarbonKredisiEntegrasyonuContract is ERC721, Ownable {
    ...
    constructor() ERC721("Emisyon Raporlama veNFT", "PRUME-3") Ownable(msg.sender) {}
...
Emisyon Raporlama ve Karbon Kredisi Entegrasyonu çözümü, üç temel aşamadan oluşan şeffaf ve otomatik bir iş akışı döngüsünü takip eder:

1. **Aşama 1 - Emisyon Ölçümü:** 
   Baca sensörü sürekli emisyon verilerini toplar ve hash değerini zincire gönderir.
2. **Aşama 2 - Karbon Vergisi Hesaplama:** 
   Akıllı sözleşme gidilen mil başına üretilen emisyonu hesaplar.
3. **Aşama 3 - Karbon Kredisi Mahsubu:** 
   Armatörün cüzdanından otomatik olarak ceza tutarında Karbon Tokeni (CO2) düşülür.
...
### Finansal Metrikler:
- **Kurulum / Geliştirme Maliyeti:** Yüksek ($48,000)
- **Operasyonel Giderler (Gas & Node):** Orta (Sensör kalibrasyon)
- **Zaman/Maliyet Tasarruf Oranı:** %100 Doğru Beyan, %90 Raporlama Zaman Tasarrufu
- **Yatırımın Amorti Süresi (ROI):** 14 Ay

### Tasarruf Formülü (ROI Hesaplama):
$$Tasarruf = (M_{geleneksel} - M_{gas}) 	imes N_{islem} - M_{kurulum}$$
Burada $M_{geleneksel}$ geleneksel bürokrasinin işlem başına yarattığı operasyonel maliyeti, $M_{gas}$ blockchain ağındaki gas ücretini, $N_{islem}$ yıllık işlem sayısını, $M_{kurulum}$ ise ilk geliştirme bütçesini ifade eder. Sağlanan **%100 Doğru Beyan, %90 Raporlama Zaman Tasarrufu** seviyesindeki verimlilik artışı ile sistem, kendini **14 Ay** gibi kısa bir sürede finanse etmektedir.

---

## 6. Küresel Başarı Hikayesi ve Sektörel Canlı Örnekler
Bu vizyonel çözüm sadece teoride kalmayıp, küresel düzeyde devasa ölçekte test edilmiştir. En bilinen canlı örneği şudur: **EU ETS denizcilik karbon izleme kurallarına uyum için DNV ve denizcilik şirketleri blockchain izleme sistemleri kurmaktadır.**
