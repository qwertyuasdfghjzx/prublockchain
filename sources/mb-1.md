# Araştırma Raporu: Gemi Yatırım Ortaklığı Tokenizasyonu (Fractional Ownership)
## Piri Reis Üniversitesi Blockchain Akademisi ve Web3 Hub
**Bölüm:** Denizcilik İşletmeleri Yönetimi  
**Ders Kimliği:** PRU-MB-1  

---

## 1. Giriş ve Sektörel Kriz Analizi
Geleneksel endüstriyel altyapıda karşılaşılan en büyük dar boğazlardan biri, operasyonların merkezi veritabanlarına, fiziksel evrak dolaşımına ve çok sayıdaki aracı kuruma bağımlı olmasıdır. Denizcilik İşletmeleri Yönetimi disiplini özelinde bu kriz, kendisini doğrudan şu sorunlarla hissettirmektedir: **Gemi yatırımları milyonlarca dolar gerektirir; küçük yatırımcıların gemi mülkiyetine ve navlun gelirlerine ortak olması imkansızdır.**
...
Bu vizyon, Gemi Yatırım Ortaklığı Tokenizasyonu (Fractional Ownership) modelini sadece teknik bir güncelleme olmaktan çıkarıp, tüm sektörü dönüştüren stratejik bir Web3 hamlesine dönüştürür.
...
/**
 * @title Gemi Yatırım Ortaklığı Tokenizasyonu (Fractional Ownership) Finansal Akıllı Sözleşmesi
 * @notice Piri Reis Üniversitesi Blockchain Akademisi Eğitim Kaynağı.
 */
contract GemiYatrmOrtaklTokenizasyonuFractionalOwnershipContract is ERC20, Ownable {
    ...
    constructor() ERC20("Gemi Yatırım OrtaklıToken", "PRUMB-1") Ownable(msg.sender) {
...
Gemi Yatırım Ortaklığı Tokenizasyonu (Fractional Ownership) çözümü, üç temel aşamadan oluşan şeffaf ve otomatik bir iş akışı döngüsünü takip eder:

1. **Aşama 1 - Gemi SPV Şirketi:** 
   Gemi için yasal şirket kurulur ve gemi değeri kadar ERC-1400 hisse tokeni basılır.
2. **Aşama 2 - Güvenlik Token Arzı (STO):** 
   Yatırımcılar KYC süreçlerini tamamlayarak stabilcoin ile gemi tokenleri satın alır.
3. **Aşama 3 - Navlun Gelir Dağıtımı:** 
   Gemi işletildikçe elde edilen navlun kârı akıllı sözleşme ile token sahiplerine temettü (dividend) ödenir.
...
### Finansal Metrikler:
- **Kurulum / Geliştirme Maliyeti:** Yüksek ($60,000)
- **Operasyonel Giderler (Gas & Node):** Orta (Regülatör uyum maliyetleri)
- **Zaman/Maliyet Tasarruf Oranı:** %100 Yatırım Demokratikleşmesi, Gemi Finansmanı Bulma Süresinde %50 Hızlanma
- **Yatırımın Amorti Süresi (ROI):** 18 Ay

### Tasarruf Formülü (ROI Hesaplama):
$$Tasarruf = (M_{geleneksel} - M_{gas}) 	imes N_{islem} - M_{kurulum}$$
Burada $M_{geleneksel}$ geleneksel bürokrasinin işlem başına yarattığı operasyonel maliyeti, $M_{gas}$ blockchain ağındaki gas ücretini, $N_{islem}$ yıllık işlem sayısını, $M_{kurulum}$ ise ilk geliştirme bütçesini ifade eder. Sağlanan **%100 Yatırım Demokratikleşmesi, Gemi Finansmanı Bulma Süresinde %50 Hızlanma** seviyesindeki verimlilik artışı ile sistem, kendini **18 Ay** gibi kısa bir sürede finanse etmektedir.

---

## 6. Küresel Başarı Hikayesi ve Sektörel Canlı Örnekler
Bu vizyonel çözüm sadece teoride kalmayıp, küresel düzeyde devasa ölçekte test edilmiştir. En bilinen canlı örneği şudur: **Shipowner.io platformu, gemi finansmanını tokenize ederek küçük yatırımlarla gemi satın alımını kolaylaştırmıştır.**
...
- C) Yaklaşık 18 Ay içinde
- D) İlk işlemde
* *Cevap:* **C**  
*Açıklama: Finansal raporlama verimlilik kazanımları sayesinde yatırım dönüş süresinin yaklaşık 18 Ay olduğunu belirtmektedir.*
