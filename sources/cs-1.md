...
Bu vizyon, Merkeziyetsiz Kimlik (DID) Altyapısı modelini sadece teknik bir güncelleme olmaktan çıkarıp, tüm sektörü dönüştüren stratejik bir Web3 hamlesine dönüştürür.
...
/**
 * @title Merkeziyetsiz Kimlik (DID) Altyapısı Mülkiyet ve Kayıt Akıllı Sözleşmesi
 * @notice Piri Reis Üniversitesi Blockchain Akademisi Eğitim Kaynağı.
 */
contract MerkeziyetsizKimlikDIDAltyapsContract is ERC721, Ownable {
    ...
    constructor() ERC721("Merkeziyetsiz KimlikNFT", "PRUCS-1") Ownable(msg.sender) {}
...
Merkeziyetsiz Kimlik (DID) Altyapısı çözümü, üç temel aşamadan oluşan şeffaf ve otomatik bir iş akışı döngüsünü takip eder:

1. **Aşama 1 - DID Oluşturma:** 
   Liman çalışanı cüzdanı üzerinden kendine özel W3C uyumlu DID tescil eder.
2. **Aşama 2 - Yetki Tanımlama:** 
   Liman yönetimi, çalışanın DID adresine 'Gantry Crane Yetkilisi' SBT rozeti basar.
3. **Aşama 3 - Şifresiz Giriş:** 
   Çalışan vinç kontrol ekranındaki QR kodu cüzdanıyla imzalayarak oturum açar.
...
- **Operasyonel Giderler (Gas & Node):** Çok Düşük (Sıfır lisans ücreti)
- **Zaman/Maliyet Tasarruf Oranı:** %100 Şifresiz Güvenlik, Kimlik Yönetim Maliyetlerinde %80 Azalma
- **Yatırımın Amorti Süresi (ROI):** 9 Ay

### Tasarruf Formülü (ROI Hesaplama):
$$Tasarruf = (M_{geleneksel} - M_{gas}) 	imes N_{islem} - M_{kurulum}$$
Burada $M_{geleneksel}$ geleneksel bürokrasinin işlem başına yarattığı operasyonel maliyeti, $M_{gas}$ blockchain ağındaki gas ücretini, $N_{islem}$ yıllık işlem sayısını, $M_{kurulum}$ ise ilk geliştirme bütçesini ifade eder. Sağlanan **%100 Şifresiz Güvenlik, Kimlik Yönetim Maliyetlerinde %80 Azalma** seviyesindeki verimlilik artışı ile sistem, kendini **9 Ay** gibi kısa bir sürede finanse etmektedir.

---

## 6. Küresel Başarı Hikayesi ve Sektörel Canlı Örnekler
Bu vizyonel çözüm sadece teoride kalmayıp, küresel düzeyde devasa ölçekte test edilmiştir. En bilinen canlı örneği şudur: **SpruceID and Microsoft Entra Verified ID, kurumsal Web3 kimlik doğrulama standartları sunmaktadır.**
