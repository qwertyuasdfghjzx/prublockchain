# Araştırma Raporu: Akıllı Navlun Escrow (Freight Settlement) Sözleşmesi
## Piri Reis Üniversitesi Blockchain Akademisi ve Web3 Hub
**Bölüm:** Uluslararası Lojistik ve Taşımacılık  
**Ders Kimliği:** PRU-LT-4  

---

## 1. Giriş ve Sektörel Kriz Analizi
Geleneksel endüstriyel altyapıda karşılaşılan en büyük dar boğazlardan biri, operasyonların merkezi veritabanlarına, fiziksel evrak dolaşımına ve çok sayıdaki aracı kuruma bağımlı olmasıdır. Uluslararası Lojistik ve Taşımacılık disiplini özelinde bu kriz, kendisini doğrudan şu sorunlarla hissettirmektedir: **Lojistik sektöründe taşıyıcılara navlun bedellerinin ödenmesinde 90 güne varan gecikmeler ve likidite sorunları yaşanır.**
...
Bu vizyon, Akıllı Navlun Escrow (Freight Settlement) Sözleşmesi modelini sadece teknik bir güncelleme olmaktan çıkarıp, tüm sektörü dönüştüren stratejik bir Web3 hamlesine dönüştürür.
...
/**
 * @title Akıllı Navlun Escrow (Freight Settlement) Sözleşmesi Mülkiyet ve Kayıt Akıllı Sözleşmesi
 * @notice Piri Reis Üniversitesi Blockchain Akademisi Eğitim Kaynağı.
 */
contract AkllNavlunEscrowFreightSettlementSzlemesiContract is ERC721, Ownable {
    ...
    constructor() ERC721("Akıllı Navlun EscrowNFT", "PRULT-4") Ownable(msg.sender) {}
...
Akıllı Navlun Escrow (Freight Settlement) Sözleşmesi çözümü, üç temel aşamadan oluşan şeffaf ve otomatik bir iş akışı döngüsünü takip eder:

1. **Aşama 1 - Navlun Depozitosu:** 
   Yükleyici, anlaşılan navlun bedelini stabilcoin olarak akıllı sözleşmeye kilitler.
2. **Aşama 2 - Taşıma ve Varış:** 
   Taşıyıcı yükü adrese ulaştırır ve alıcıya onay kodu taratır.
3. **Aşama 3 - Bakiye Serbest Bırakma:** 
   Alıcının onayıyla sözleşme navlun tutarını taşıyıcının cüzdanına gönderir.
...
### Finansal Metrikler:
- **Kurulum / Geliştirme Maliyeti:** Orta ($24,000)
- **Operasyonel Giderler (Gas & Node):** Çok Düşük (Sadece gas fee)
- **Zaman/Maliyet Tasarruf Oranı:** %95 Ödeme Hızlanması, Finansal Mutabakat Sürelerinde %50 Azalma
- **Yatırımın Amorti Süresi (ROI):** 7 Ay

### Tasarruf Formülü (ROI Hesaplama):
$$Tasarruf = (M_{geleneksel} - M_{gas}) 	imes N_{islem} - M_{kurulum}$$
Burada $M_{geleneksel}$ geleneksel bürokrasinin işlem başına yarattığı operasyonel maliyeti, $M_{gas}$ blockchain ağındaki gas ücretini, $N_{islem}$ yıllık işlem sayısını, $M_{kurulum}$ ise ilk geliştirme bütçesini ifade eder. Sağlanan **%95 Ödeme Hızlanması, Finansal Mutabakat Sürelerinde %50 Azalma** seviyesindeki verimlilik artışı ile sistem, kendini **7 Ay** gibi kısa bir sürede finanse etmektedir.

---

## 6. Küresel Başarı Hikayesi ve Sektörel Canlı Örnekler
Bu vizyonel çözüm sadece teoride kalmayıp, küresel düzeyde devasa ölçekte test edilmiştir. En bilinen canlı örneği şudur: **DexFreight platformu, kamyon taşımacılığında akıllı sözleşme tabanlı anlık navlun ödemeleri gerçekleştirmektedir.**
...
- C) Yaklaşık 7 Ay içinde
- D) İlk işlemde
* *Cevap:* **C**  
*Açıklama: Finansal raporlama verimlilik kazanımları sayesinde yatırım dönüş süresinin yaklaşık 7 Ay olduğunu belirtmektedir.*
