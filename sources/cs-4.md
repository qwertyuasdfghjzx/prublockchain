...
Bu vizyon, Dağıtık Dosya Depolama Entegrasyonu (IPFS/Arweave) modelini sadece teknik bir güncelleme olmaktan çıkarıp, tüm sektörü dönüştüren stratejik bir Web3 hamlesine dönüştürür.
...
/**
 * @title Dağıtık Dosya Depolama Entegrasyonu (IPFS/Arweave) Mülkiyet ve Kayıt Akıllı Sözleşmesi
 * @notice Piri Reis Üniversitesi Blockchain Akademisi Eğitim Kaynağı.
 */
contract DatkDosyaDepolamaEntegrasyonuIPFSArweaveContract is ERC721, Ownable {
    ...
    constructor() ERC721("Dağıtık Dosya DepolaNFT", "PRUCS-4") Ownable(msg.sender) {}
...
Dağıtık Dosya Depolama Entegrasyonu (IPFS/Arweave) çözümü, üç temel aşamadan oluşan şeffaf ve otomatik bir iş akışı döngüsünü takip eder:

1. **Aşama 1 - Dosya Yükleme:** 
   Gemi enspektörü hasar fotoğraflarını portal üzerinden yükler.
2. **Aşama 2 - IPFS CID Üretimi:** 
   Dosya IPFS'e atılır ve benzersiz CID (örn: QmXoyp...) adresi alınır.
3. **Aşama 3 - Sözleşmeye Kilitlenme:** 
   CID adresi hasar tespit akıllı sözleşmesine kaydedilerek sonsuza kadar kilitlenir.
...
- **Operasyonel Giderler (Gas & Node):** Çok Düşük (AWS S3 maliyetlerinden %90 daha ucuz)
- **Zaman/Maliyet Tasarruf Oranı:** %90 Depolama Maliyet Tasarrufu, %100 Veri Kayıp Koruması
- **Yatırımın Amorti Süresi (ROI):** 3 Ay

### Tasarruf Formülü (ROI Hesaplama):
$$Tasarruf = (M_{geleneksel} - M_{gas}) 	imes N_{islem} - M_{kurulum}$$
Burada $M_{geleneksel}$ geleneksel bürokrasinin işlem başına yarattığı operasyonel maliyeti, $M_{gas}$ blockchain ağındaki gas ücretini, $N_{islem}$ yıllık işlem sayısını, $M_{kurulum}$ ise ilk geliştirme bütçesini ifade eder. Sağlanan **%90 Depolama Maliyet Tasarrufu, %100 Veri Kayıp Koruması** seviyesindeki verimlilik artışı ile sistem, kendini **3 Ay** gibi kısa bir sürede finanse etmektedir.

---

## 6. Küresel Başarı Hikayesi ve Sektörel Canlı Örnekler
Bu vizyonel çözüm sadece teoride kalmayıp, küresel düzeyde devasa ölçekte test edilmiştir. En bilinen canlı örneği şudur: **Filecoin and Arweave, dünya genelinde petabaytlarca kurumsal veriyi merkeziyetsiz depolama ağlarında saklamaktadır.**
