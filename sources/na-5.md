# Araştırma Raporu: Gemi Dijital İkizi Blokzincir Senkronizasyonu
## Piri Reis Üniversitesi Blockchain Akademisi ve Web3 Hub
**Bölüm:** Gemi İnşaatı ve Gemi Makineleri Mühendisliği  
**Ders Kimliği:** PRU-NA-5  

---

## 1. Giriş ve Sektörel Kriz Analizi
Geleneksel endüstriyel altyapıda karşılaşılan en büyük dar boğazlardan biri, operasyonların merkezi veritabanlarına, fiziksel evrak dolaşımına ve çok sayıdaki aracı kuruma bağımlı olmasıdır. Gemi İnşaatı ve Gemi Makineleri Mühendisliği disiplini özelinde bu kriz, kendisini doğrudan şu sorunlarla hissettirmektedir: **Gemi işletilirken yapılan modifikasyonlar ve parça değişimleri tasarım dosyalarına (As-Built) yansıtılmaz ve dijital ikiz güncelliğini yitirir.**
...
Bu vizyon, Gemi Dijital İkizi Blokzincir Senkronizasyonu modelini sadece teknik bir güncelleme olmaktan çıkarıp, tüm sektörü dönüştüren stratejik bir Web3 hamlesine dönüştürür.
...
/**
 * @title Gemi Dijital İkizi Blokzincir Senkronizasyonu Mülkiyet ve Kayıt Akıllı Sözleşmesi
 * @notice Piri Reis Üniversitesi Blockchain Akademisi Eğitim Kaynağı.
 */
contract GemiDijitalkiziBlokzincirSenkronizasyonuContract is ERC721, Ownable {
    ...
    constructor() ERC721("Gemi Dijital İkizi BNFT", "PRUNA-5") Ownable(msg.sender) {}
...
Gemi Dijital İkizi Blokzincir Senkronizasyonu çözümü, üç temel aşamadan oluşan şeffaf ve otomatik bir iş akışı döngüsünü takip eder:

1. **Aşama 1 - Tersane Modifikasyonu:** 
   Gemideki bir pompa değiştirildiğinde yeni CAD çizimleri sisteme yüklenir.
2. **Aşama 2 - On-Chain Sürüm Güncelleme:** 
   Akıllı sözleşme, dijital ikiz dosya hash'ini yeni sürümüne günceller.
3. **Aşama 3 - Eşzamanlı Bildirim:** 
   Klas ve donatan, geminin en son güncel yapısını anında zincirden çeker.
...
### Finansal Metrikler:
- **Kurulum / Geliştirme Maliyeti:** Çok Yüksek ($75,000)
- **Operasyonel Giderler (Gas & Node):** Orta (Bant veri senkronizasyonu)
- **Zaman/Maliyet Tasarruf Oranı:** %100 Güncel Dijital İkiz, %30 Bakım Planlama Verimliliği
- **Yatırımın Amorti Süresi (ROI):** 18 Ay

### Tasarruf Formülü (ROI Hesaplama):
$$Tasarruf = (M_{geleneksel} - M_{gas}) 	imes N_{islem} - M_{kurulum}$$
Burada $M_{geleneksel}$ geleneksel bürokrasinin işlem başına yarattığı operasyonel maliyeti, $M_{gas}$ blockchain ağındaki gas ücretini, $N_{islem}$ yıllık işlem sayısını, $M_{kurulum}$ ise ilk geliştirme bütçesini ifade eder. Sağlanan **%100 Güncel Dijital İkiz, %30 Bakım Planlama Verimliliği** seviyesindeki verimlilik artışı ile sistem, kendini **18 Ay** gibi kısa bir sürede finanse etmektedir.

---

## 6. Küresel Başarı Hikayesi ve Sektörel Canlı Örnekler
Bu vizyonel çözüm sadece teoride kalmayıp, küresel düzeyde devasa ölçekte test edilmiştir. En bilinen canlı örneği şudur: **GE Digital and Siemens, endüstriyel IoT ve dijital ikiz veri doğruluğu için blockchain mimarileri kurmaktadır.**
