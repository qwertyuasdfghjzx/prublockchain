...
Bu vizyon, Çelik Sac Kalınlığı ve Menşei Doğrulama modelini sadece teknik bir güncelleme olmaktan çıkarıp, tüm sektörü dönüştüren stratejik bir Web3 hamlesine dönüştürür.
...
/**
 * @title Çelik Sac Kalınlığı ve Menşei Doğrulama Mülkiyet ve Kayıt Akıllı Sözleşmesi
 * @notice Piri Reis Üniversitesi Blockchain Akademisi Eğitim Kaynağı.
 */
contract elikSacKalnlveMeneiDorulamaContract is ERC721, Ownable {
    ...
    constructor() ERC721("Çelik Sac Kalınlığı NFT", "PRUNA-2") Ownable(msg.sender) {}
...
Çelik Sac Kalınlığı ve Menşei Doğrulama çözümü, üç temel aşamadan oluşan şeffaf ve otomatik bir iş akışı döngüsünü takip eder:

1. **Aşama 1 - Çelik Döküm Girişi:** 
   Metalürji laboratuvarı çelik plaka analiz sertifikasını zincire yazar.
2. **Aşama 2 - Tersane Kabulü:** 
   Tersaneye gelen saclar ultrasonik ölçümle taranır, veriler onchain doğrulanır.
3. **Aşama 3 - Gemi Gövde Haritası:** 
   Sacın geminin hangi postasında kullanıldığı dijital ikiz üzerinde DLT'ye işlenir.
...
### Finansal Metrikler:
- **Kurulum / Geliştirme Maliyeti:** Yüksek ($42,000)
- **Operasyonel Giderler (Gas & Node):** Orta (Lazer etiketleme)
- **Zaman/Maliyet Tasarruf Oranı:** %100 Mukavemet Güvencesi, Sörvey İnceleme Kolaylığı
- **Yatırımın Amorti Süresi (ROI):** 15 Ay

### Tasarruf Formülü (ROI Hesaplama):
$$Tasarruf = (M_{geleneksel} - M_{gas}) 	imes N_{islem} - M_{kurulum}$$
Burada $M_{geleneksel}$ geleneksel bürokrasinin işlem başına yarattığı operasyonel maliyeti, $M_{gas}$ blockchain ağındaki gas ücretini, $N_{islem}$ yıllık işlem sayısını, $M_{kurulum}$ ise ilk geliştirme bütçesini ifade eder. Sağlanan **%100 Mukavemet Güvencesi, Sörvey İnceleme Kolaylığı** seviyesindeki verimlilik artışı ile sistem, kendini **15 Ay** gibi kısa bir sürede finanse etmektedir.

---

## 6. Küresel Başarı Hikayesi ve Sektörel Canlı Örnekler
Bu vizyonel çözüm sadece teoride kalmayıp, küresel düzeyde devasa ölçekte test edilmiştir. En bilinen canlı örneği şudur: **Tersaneler ve klas kuruluşları, malzeme kalitesini izlemek için Hyperledger tabanlı çelik izlenebilirlik ağları kullanmaktadır.**
